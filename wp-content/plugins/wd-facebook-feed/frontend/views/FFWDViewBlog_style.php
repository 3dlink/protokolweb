...
class FFWDViewBlog_style {
  ...
  public function display($params, $from_shortcode = 0, $ffwd = 0) {
    ...
    $ffwd_data = $this->model->get_ffwd_data($params['fb_id'], $ffwd_info['objects_per_page'], /*$ffwd_info['sort_by']*/'', $ffwd, /*$order_by*/ ' ASC');
    if (!$ffwd_data) {
												...
												<div class="ffwd_title_spun1_<?php echo $ffwd; ?>">
													...
													if($show_comments || $show_likes || $show_shares) { ?>
														<div class="ffwd_comments_likes_<?php echo $ffwd; ?>" style="float:left">
															...
															if($show_likes) {
																?>
																<div id="ffwd_likes_<?php echo $ffwd . '_' . $ffwd_data_row->id; ?>" class="ffwd_likes_<?php echo $ffwd; ?>" style="display : <?php echo ($ffwd_data_row->type == "events") ? 'none': 'block'; ?>">
																</div>
																<?php
															}
															if($show_shares) {

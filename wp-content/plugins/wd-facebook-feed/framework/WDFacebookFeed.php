...
////////////////////////
  ...
  public static function timeline() {
		...
		*/
		...
		$data = array();
		$fields = 'fields=id,name,story,link,created_time,updated_time,from,message,type,source,place,message_tags,with_tags,story_tags,description,status_type&';
		$edge = (self::$fb_type == 'group') ? 'feed' : (self::$timeline_type == 'feed' || self::$timeline_type == 'others') ? 'feed' : 'posts';
		$fb_graph_url = str_replace (
      array('{FB_ID}', '{EDGE}','{ACCESS_TOKEN}', '{FIELDS}', '{LIMIT}', '{OTHER}'),
      array(self::$id, $edge, 'access_token='.self::$access_token.'&', $fields, 'limit=100&locale='.get_locale().'&', ''),
      self::$graph_url
    );
  ...
  public static function specific() {
    ...
    if(self::$content[0] == 'albums')
      $fields = 'fields=name,created_time,updated_time,from,link,count&';
    elseif(self::$content[0] == 'photos')
      $fields = 'fields=source,name,created_time,updated_time,from,link,images&';
    elseif(self::$content[0] == 'videos')
      // $fields = 'fields=source,name,created_time,updated_time,from,description,format&';
      $fields = 'fields=source,created_time,updated_time,from,description,format&';
		elseif(self::$content[0] == 'events')
      $fields = 'fields=name,start_time,end_time,description,cover,owner,place&';
    else
      $fields = 'fields=source,name,story,created_time,updated_time,from,link&';

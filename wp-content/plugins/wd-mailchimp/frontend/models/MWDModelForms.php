...
"#mwd-form".$form_id."{
	...
	box-shadow:".$form_theme['AGPBoxShadow']."px;
...
#mwd-popover".$form_id." .mwd-popover-content {
	width:".$form_theme['AGPSPWidth']."% !important;
	margin:".$form_theme['AGPMargin'].";
	position: relative;
}";
...
#minimize-form".$form_id." {
	top: ".$form_theme['CBPTop'].";
	".$for_mini.": ".(2 * $form_theme['CBP'.ucfirst($for_mini)] + $form_theme['CBPFontSize'])."px;
	bottom: ".$form_theme['CBPBottom'].";
}";
	...
	public function savedata($form, $id) {
		...
		if (isset($_POST["mwd-counter" . $id])) {
			...
			elseif (isset($_POST["mwd_arithmetic_captcha_input"])) {
				...
				$session_mwd_arithmetic_captcha_code = isset($_SESSION[$id . '_mwd_arithmetic_captcha_code']) ? $_SESSION[$id . '_wd_arithmetic_captcha_code'] : '-';
		...
		foreach ($label_type as $key => $type) {
			...
			if(!in_array($i, $disabled_fields)) {
				...
				if(in_array($i, $merge_vars)){
					...
					$merge_vars[$merge_var_key] = str_replace(array("*@@url@@*", "***grading***", "***br***"), array(" ", "", " ", " ", " ", ", "), addslashes($value));
				}

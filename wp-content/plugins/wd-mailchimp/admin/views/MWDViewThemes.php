...
class MWDViewThemes {
	...
	public function display() {
		...
		<div class="mwd-user-manual">
			...
			<a target="_blank" href="https://web-dorado.com/wordpress-mailchimpwd-guid/themes.html">Read More in User Manual</a>
		</div>
		...
		<form class="wrap" id="themes_form" method="post" action="admin.php?page=themes">
	...
	public function edit($id, $reset) {
		...
		$active_tab = isset($_REQUEST["active_tab"]) && $_REQUEST["active_tab"] ? $_REQUEST["active_tab"] : 'custom_css';
		...
		<div ng-app="ThemeParams">
			<div ng-controller="MWDTheme">
				<form id="mwd-themes-form" method="post" action="admin.php?page=themes" style="width:99%;" >
					<?php wp_nonce_field('nonce_mwd', 'nonce_mwd'); ?>
							...
							<div class="col-md-6 col-sm-5">
								...
								<div class="mwd-themes-tabs col-md-12">
									...
									<div class="mwd-themes-tabs-container">
										<div class="mwd-error col-md-12">With free version you can alter form display with Custom CSS only. Get elegant and user-friendly theme settings with MailChimp WD Pro.</div>
										<?php
										$k = 0;
										foreach($all_params as $pkey => $params) {
											$show_hide_class = $pkey == 'custom_css'  ? '' : 'mwd-hide';
											$free_pro =  $pkey == 'custom_css' ? '-free' : '';
											echo '<div id="'.$pkey.'-content'.$free_pro.'" class="mwd-themes-container '.$show_hide_class.'">';
											...
											echo '</div>';
											if($pkey != 'custom_css'){
												$show_hide_class_image = $active_tab == $pkey ? '' : 'mwd-hide';
												echo '<div id="'.$pkey.'-content-free" class="mwd-themes-container '.$show_hide_class_image.'">
													<img src="'.MWD_URL.'/assets/'.$pkey.'.png" />
												</div>';
											}
										} 
		...
		<script>
			...
			jQuery(".mwd-themes-tabs li a").click(function(){
				...
				jQuery("#"+jQuery(this).attr("id")+'-content-free').show();

...
class MWDViewSubmissions {
	...
	public function display($form_id) {
		...
		<div class="mwd-user-manual">
			...
			<a target="_blank" href="https://web-dorado.com/wordpress-mailchimpwd-guid/submissions.html">Read More in User Manual</a>
		</div>
		...
		<form action="admin.php?page=submissions" method="post" id="admin_form" name="admin_form">
			<?php wp_nonce_field('nonce_mwd', 'nonce_mwd'); ?>
						...
						for ($www = 0, $qqq = count($group_id_s); $www < $qqq; $www++) {
							...
							<tr <?php echo $alternate; ?>>
								<td  class="table_small_col submitdate_fc sub-align" id="view_submissions">
									<a href="<?php echo add_query_arg(array('action' => 'view_submits', 'page'=> 'submissions', 'task' => 'view_submit', 'form_id' => $data->form_id, 'group_id' => $data->group_id, 'nonce_mwd_ajax' => wp_create_nonce('nonce_mwd_ajax'), 'width' => '1000', 'height' => '500', 'TB_iframe' => '1'), admin_url('admin-ajax.php')); ?>"
								...
								<td class="table_medium_col_uncenter submitterip_fc sub-align" id="submitterip_fc" <?php echo $style_ip; ?>>
									<a href="<?php echo add_query_arg(array('action' => 'submitter_ip', 'page'=> 'submissions', 'task' => 'view_ip', 'data_ip' => $data->ip, 'nonce_mwd_ajax' => wp_create_nonce('nonce_mwd_ajax'), 'width' => '1000', 'height' => '500', 'TB_iframe' => '1'), admin_url('admin-ajax.php')); ?>"
									   class="thickbox thickbox-preview"
					...
					if ($is_stats) {
						...
						function show_stats() {
							...
							if(jQuery('#sorted_label_key').val()!="") {
								jQuery('#div_stats').load('<?php echo add_query_arg(array('action' => 'get_stats', 'page' => 'submissions'), admin_url('admin-ajax.php')); ?>', {
									'task': 'show_stats',
